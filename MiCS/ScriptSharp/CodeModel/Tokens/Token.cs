...
namespace ScriptSharp.CodeModel {
    ...
    public class Token : IComparable {
        ...
        public const int MaximumPrecedence = int.MaxValue - 1;

        private static TokenInfo[] infos;
        ...
        public int CompareTo(Token token) {
            return _position.CompareTo(token.Position);
        }
        ...
        public bool IsAdjacent(Token right) {
            return (_position.Line == right.Position.Line) &&
                   ((_position.Column + this.ToString().Length) == right.Position.Column);
        }
